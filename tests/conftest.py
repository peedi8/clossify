...
실측(도입 전): 전체 스위트 소켓 연결 3,886 회 → 그중 3,882 회는
...
pyproject 의 testcfg 가 자동 적용). 도입 시점에 이 예외를 요청하는 테스트는
    ...
    **실패가 성공으로 위장** 되므로(우리가 반복해 막아온 유형) 절대 잡지 마라.
    ...
    def __init__(self, address):
        ...
        super().__init__(
            ...
            f"만들지 마세요 (2026-08-08 네이버 403 사고 재발 방지 — 테스트 외부 호출 차단). "
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_marker = pytest.mark.skip(
        reason=(
            "allow_external_network 마커 — 기본 실행에서 제외됨(테스트 외부 호출 차단). "
...
def allow_external_network():
    ...
    **도입 시점에 사용처가 없다.** 마커와 함께 쓰이며, 이 픽스처를 요청한
...
def socket_guard_off():
    ...
    용도로 쓰면 테스트 외부 호출 차단 계약을 우회하는 것이 된다.
