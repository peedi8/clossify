...
class TestToolCount:
    ...
    def test_g_exactly_ten_mcp_tools(self):
        """(g) ``@mcp.tool()`` 데코레이터가 10개, runtime list_tools() 도 10개."""
        # 소스 카운트.
        src = Path(mcp_server.__file__).read_text(encoding="utf-8")
        decorator_count = src.count("@mcp.tool()")
        assert decorator_count == 10, f"@mcp.tool() 데코레이터가 10개여야 함: {decorator_count}"
        # 런타임 카운트.
        tools = _list_tools()
        assert len(tools) == 10, f"runtime 도구가 10개여야 함: {len(tools)}"
