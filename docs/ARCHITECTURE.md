...
컴플라이언스 결정론 검사(qa_agents._compliance_code_check)
   ├─ 원본 이미지(대표 이미지 URL 이 빈 문자열/공백이면 FAIL — 진입 게이트.
   │  이미지가 존재하는지만 판정하며 사진의 진위·출처는 검증하지 않는다.)
...
| `FAIL` | 결정론적 위반(금지 표현/고시 필수 누락/원산지·KC 위반/AS 연락처 누락/KC 필요 여부 불명/원본 이미지 누락) | **차단** |
