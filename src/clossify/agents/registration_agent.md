...
2. **태그(sellerTags)** — 제목에 못 넣은 **관련** 키워드 스프레드 ~10개(롱테일 포함: 탁상화병·미니화병·홈데코). 제네릭/타카테고리/색덤프 금지. **네이버가 제한어(restricted) 판정 시 자동으로 제거하고 재시도** — 이 동작은 `naver_client` 가 페이로드 송신 시 수행한다.
   ★ **무엇이 걸러지고 무엇이 안 걸러지는지 정확히 알아라** (실측 2026-08-14):
   - **제한어**: 송신 시 자동 제거 + 재시도 (`naver_client`).
   - **상품명·브랜드·카테고리명과 겹치는 태그**: `prepare_listing` 경로에서 자동 제거되고
     `field_duplicates` 로 보고된다 (`register._resolve_tags` → `filter_duplicate_field_tags`).
   - **제조사·수입자와 겹치는 태그, 그리고 목록 안의 자기 중복**: **아무도 안 걸러준다.**
     검수가 경고로 드러낼 뿐 태그는 그대로 나간다. → **겹치지 않게 만드는 건 네 책임이다.**
   - `prepare_listing` 을 **거치지 않고** `register_product` 를 직접 부르면 위의 필터가
     아예 안 돈다 — 네가 준 태그가 그대로 간다.
   네이버 태그 규칙: 브랜드·제조사·판매처는 각자 제 필드에 들어가므로 태그에 또 넣지 않는다.
