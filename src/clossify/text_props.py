...
BANNED_CLAIM_RE = re.compile(
    ...
    # ── 한글 조각: 한국어판 단어 경계 (N86/T3 검증 방식) ──
    # 각 한글 금지 조각을 ``(?<![가-힣]) … (?![가-힣])`` 로 감싼다 —
    # 앞·뒤가 한글이 아니어야(문자열 시작/끝·공백·숫자·영문·구두점)
    # 매치한다. Python \b 는 한글을 단어문자로 취급하여 한글 경계가
    # 안 생기기 때문에 이 방식이 필수다.
    #
    ...
    r"(?<![가-힣])공식(?![가-힣])|"
    # 순위 주장: ``\d\s*위(?![가-힣])`` — 앞쪽은 lookbehind 없이
