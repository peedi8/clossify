...
BANNED_CLAIM_RE = re.compile(
    ...
    r"완벽(?:한|하게)?|"
    r"프리미엄|"
    # 규칙문서가 선언한 금지표현을 코드가 잡도록 넓힘.
    # 각 항목의 과탐 위험은 아래 주석 + COMPLIANCE_RULES.md §1 비고 참조.
    r"공\s*식(?!품)|"  # "공식" 단 (?!품) 로 가공식품 등 보호.
    # 순위 주장: 앞이 아니라 뒤에서 판별한다.
    # 앞쪽 lookbehind (?<![가-힣0-9]) 는 한글을 단어문자로 취급하여
    # "업계1위"·"국내1위"·"판매1위" 같은 무공백 형태를 통과시켰다.
    # \d\s*위(?![가-힣]) 는 "위" 뒤에 한글이 바로 붙으면(3위생·1위생용품)
    # 통과시킨다. 단 "1위상품"·"100위권" 도 못 잡는다 — 이것은
    # 정상어(1위생용품) 와 구별할 수 없기 때문이며, COMPLIANCE_RULES.md §1
    # 비고에 명시했다. 잡는 척하지 않는다.
    r"\d\s*위(?![가-힣])|"
    # 영문 BEST: Python \b 는 한글을 단어문자로 취급하여 "BEST상품" 에서
    # 경계가 안 생긴다. 영문자/숫자 경계로만 판별한다.
    r"(?<![A-Za-z0-9])BEST(?![A-Za-z0-9])|"  # 영문 BEST (베스트 조끼와 무관).
    r"최저\s*가|초\s*특가|역대급|"  # 가격 배타 주장.
    # 배타 주장. "업계 1위" 갈래의 \d\s*위 패턴은 위에서 흡수했으므로
    # 중복 제거 — 업계/국내/세계 수식어만 남긴다.
    r"국내\s*유일|세계\s*최초|"
    r"무조건",  # 단정 표현.
    re.IGNORECASE,
)
