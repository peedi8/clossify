...
def _rank_tail():
    ...
    ``_kr_tail()`` 조건에 더해 ``상품`` 이 뒤따를 때도 매치한다.
    WO 6라운드: ``1위상품권`` 의 ``위`` 뒤 ``상`` 이 비조사 한글이어서
    ``_kr_tail()`` 만으로는 놓쳤다. 통제군 보호 대상은 ``위`` + ``생``
    (``1위생용품``·``3위생 마스크``) 뿐이므로, ``상품`` 을 매치 조건에
    추가해도 과탐 위험이 없다.
    """
    return r"(?:(?![가-힣])|(?=" + _KOREAN_PARTICLE_OR_ENDING + r")|(?=상품))"
...
BANNED_CLAIM_RE = re.compile(
    ...
    # ``국내최고 상품`` …) 을 통째로 놓쳤다(미탐 대량). 컴플라이언스 게이트는
    # 미탐이 오탐보다 비싸므로(상설 조항), 앞쪽 한글 전체 차단을 걷어낸다.
    # 대신 **통제군에서 확인된 보호 접두사만** lookbehind 로 배제한다:
    #   - ``비공식`` → ``(?<!비)공식`` (부정 접두사 ``비`` 배제)
    # 나머지 금지 조각(``정품``·``최고``·``최저가`` …) 은 통제군에
    # 한글 접두사 결합형 보호 대상이 없으므로 앞쪽 경계를 쓰지 않는다.
    # 보호 복합명사(``정품인증서``·``가공식품``·``한정식``) 는 뒤쪽
    # ``_kr_tail()`` 이 이미 지키고 있다 — ``인``·``품``·``식`` 뒤 한글이
    # 조사·어미가 아니므로 비매치.
    #
    # **뒤쪽 경계 (WO 4라운드)**: ``_kr_tail()`` 로 비한글이거나 조사·어미로
    # 시작할 때만 매치한다 (상세는 ``_kr_tail()`` docstring 참조).
    #
    # **순위 ``\d\s*위`` 의 뒤쪽 경계 (WO 6라운드)**: ``_kr_tail()`` 만으로는
    # ``1위상품권`` 의 ``위`` 뒤 ``상`` (비조사 한글) 이 막아서 놓쳤다.
    # 통제군 보호는 ``위`` + ``생`` (``1위생용품``·``3위생 마스크``) 뿐이므로,
    # ``상품`` 이 뒤따를 때는 매치하도록 ``_rank_tail()`` 을 쓴다.
    #
    # **공백 삽입 회피 (WO 7라운드)**: 필터를 아는 판매자가 ``정 품``·
    # ``진 품``·``최 고 급`` 처럼 글자 사이에 공백을 넣어 회피한다.
    # 3라운드에서 ``\s*`` 제거로 과탐을 고치면서 이 회피 적발도 같이
    # 사라졌다(6R 실측 회귀). 두 가지를 양립시킨다:
    #
    #   1. 공백 회피 갈래: ``(?<![가-힣])정\s+품`` — 첫 글자가 한글이 아닌
    #      것(문자열 시작·공백·구두점·영문·숫자) 으로 시작할 때만 매치.
    #      이 lookbehind 가 ``수정 품질`` 의 ``정`` (앞 글자 ``수`` 한글)
    #      을 막아 정상 문구를 보호한다.
    #   2. 무공백 갈래: ``정품`` — ``\s*`` 없이 원래 낱말만 매치.
    #      6R 앞쪽 경계 제거로 ``업계정품`` 같은 수식어 결합도 잡는다.
    #
    # ``최\s+고\s+급`` (WO 7라운드): ``최 고 급`` (모든 글자 공백) 은
    # 회피이지만 ``최 고급`` (``최`` 뒤만 공백) 은 ``최고급`` 의 자연스러운
    # 띄어쓰기(예: ``최 고급 호텔``) 이므로 잡지 않는다. ``\s+급`` 으로
    # ``고`` 와 ``급`` 사이에도 공백이 있을 때만 매치.
    r"(?<![가-힣])정\s+품" + _kr_tail() + r"|"
    r"(?<![가-힣])진\s+품" + _kr_tail() + r"|"
    r"(?<![가-힣])최\s+고\s+급" + _kr_tail() + r"|"
    r"(?<![가-힣])최\s+고(?!\s*급)" + _kr_tail() + r"|"
    r"정품" + _kr_tail() + r"|"
    r"진품" + _kr_tail() + r"|"
    r"최고(?:급)?" + _kr_tail() + r"|"
    r"최상급" + _kr_tail() + r"|"
    r"완벽(?:한|하게)?" + _kr_tail() + r"|"
    r"프리미엄" + _kr_tail() + r"|"
    # ``공식`` — ``(?<!비)`` 로 ``비공식`` (부정 접두사) 만 배제한다.
    # ``가공식품`` (앞 ``가``) 은 ``공식`` 의 ``식`` 뒤 ``품`` 이 조사가
    # 아니어서 ``_kr_tail()`` 이 이미 보호한다. 3라운드의 ``(?<![가-힣])``
    # 전체 차단은 불필요하게 넓었고, ``업계공식`` 같은 수식어 결합을
    # 놓쳤다 — WO 6라운드에서 좁힌다.
    r"(?<!비)공식" + _kr_tail() + r"|"
    # 순위 주장: ``\d\s*위`` — 앞쪽은 lookbehind 없이
    # (``업계1위``·``국내1위`` 같은 무공백 형태를 잡기 위해).
    # 뒤쪽은 ``_rank_tail()``: ``_kr_tail()`` 조건에 더해 ``상품`` 이
    # 뒤따를 때도 매치(``1위상품권`` 적발). ``위`` + ``생`` (``1위생용품``·
    # ``3위생``) 은 ``생`` 이 비조사 한글이고 ``상품`` 도 아니므로 보호된다.
    r"\d\s*위" + _rank_tail() + r"|"
    # 가격 배타 주장. ``최저가`` 는 한 낱말이므로 ``\s*`` 제거.
    # (구 ``최저\s*가`` 는 ``최저 가지색`` 의 ``가`` 를 잡아 과탐.)
    r"최저가" + _kr_tail() + r"|"
    # ``초특가``·``초 특가`` 둘 다 잡아야 하므로 ``\s*`` 유지.
    # ``초 특별 솔루션`` 은 ``특`` 뒤 ``가`` 가 아니므로 안 걸림.
    r"초\s*특가" + _kr_tail() + r"|"
    r"역대급" + _kr_tail() + r"|"
    # 배타 주장. ``국내 유일``·``국내유일``·``세계 최초``·``세계최초`` 를
    # 잡기 위해 ``\s*`` 유지.
    r"국내\s*유일" + _kr_tail() + r"|"
    r"세계\s*최초" + _kr_tail() + r"|"
    # 표시광고법 위험 가격 표현 (T207): ``국내최저``·``최저가격``·``가격파괴``
    # 계열. ``유일`` 은 정상 문맥(``유일무이``) 오탐 위험으로 범위 제외.
    #
    # **T207 수리(오탐 차단)**: ``_kr_tail()`` 은 뒤 글자가 조사 목록
    # (``임``·``요``·``이``·``가`` …) 으로 시작하면 통과시키므로, 합성어의
    # 다음 음절이 조사와 겹칠 때만 오탐이 터진다(``국내최저임금`` →
    # ``임금``·``국내최저요금`` → ``요금``·``국내최저이자`` → ``이자``).
    # ``국내\s*최저`` 는 수식어형이라 공백 뒤 한글 낱말(``국내 최저 기온``)
    # 에도 걸린다. ``_kr_tail()`` 자체는 다른 금지어가 의존하므로 건드리지
    # 않고, **이 항목에만 적용되는 예외 목록**(negative lookahead) 으로
    # 막는다. 예외 목록은 WO 통제군에서 실측 확인된 정상 합성어 머리만
    # 닫힌 집합으로 둔다: ``임금``·``요금``·``이자``·``이율``·``가격``·``기온``.
    # (``선반``·``급``·``수준`` 계열은 다음 음절이 애초에 조사가 아니어서
    # ``_kr_tail()`` 이 이미 보호한다.)
    #
    # **접두 우선(잔여물 방지)**: alternation 은 왼쪽이 우선이므로 더 긴
    # 가격 표현부터 소비한다. ``국내 최저가격 보장`` 을 ``국내\s*최저`` 가
    # 먼저 먹으면 ``가격 보장`` 이, ``국내 최저가 보장`` 을 먹으면
    # ``가 보장`` 이 남는다. ``국내\s*최저\s*가격`` → ``국내\s*최저가`` →
    # ``국내\s*최저`` 순서로 긴 것부터 나열해 전부 소비한다.
    #
    # **음절 공백 회피**: 기존 ``정\s+품``·``최\s+고\s+급`` (WO 7라운드)
    # 방식을 그대로 따른다 — 첫 글자 앞이 한글이 아니고(``(?<![가-힣])``)
    # 글자 사이에 공백이 있을 때(``\s+``) 만 매치하는 별도 갈래.
    # ``국내 최 저``·``최 저 가격``·``최저 가 격``·``가격 파 괴`` 적발용.
    r"국내\s*최저\s*가격" + _kr_tail() + r"|"
    r"국내\s*최저가(?!\s*격)" + _kr_tail() + r"|"
    r"국내\s*최저(?!\s*(?:임금|요금|이자|이율|가격|기온))" + _kr_tail() + r"|"
    r"최저\s*가격" + _kr_tail() + r"|"
    r"가격\s*파괴" + _kr_tail() + r"|"
    r"(?<![가-힣])국내\s+최\s+저" + _kr_tail() + r"|"
    r"(?<![가-힣])최\s+저\s*가\s*격" + _kr_tail() + r"|"
    r"(?<![가-힣])최저\s+가\s+격" + _kr_tail() + r"|"
    r"(?<![가-힣])가격\s+파\s+괴" + _kr_tail() + r"|"
    r"무조건" + _kr_tail(),
    re.IGNORECASE,
)
