...
원산지·제조사·수입사·모델명·AS 전화는 `.local/config.json` 의
`smartstore_notice_defaults` 에 한 번 넣어두면 다시 묻지 않는다.

위의 공통 5가지는 아직 설정 자리가 없어서 상품마다 묻는다. 같은 대화 안에서라면
"아까 답한 대로 써줘"로 넘어갈 수 있다.
